...
1. **Layout / UI-SPEC for Shot Capture**
   - What we know: Phase 1 UI-SPEC covers Setup only; Phase 2 has no formal UI-SPEC yet
   - What's unclear: Exact button arrangement (3+2 rows? 2+2+1? All 5 in one row?), score bar position, whether hole nav is a drawer, strip, or modal
   - Recommendation: Planner should either create a Phase 2 UI-SPEC (as Phase 1 did) or document layout decisions in plan tasks. The brutalist design system (BrdyColors, BrdySpacing, BrdyTextTheme) is locked — only layout arrangement is open.

2. **A3 — Stroke Index availability**
   - What we know: Phase 1 flagged this as unverified [ASSUMED]; `HoleModel.strokeIndex` is nullable; SI may or may not be returned by the API
   - What's unclear: What percentage of courses in `golfcourseapi.com` have SI data; whether the Hive-cached `CourseModel.holes[i].strokeIndex` is ever non-null
   - Recommendation: SHOT-11 always shows SI if available, "–" if null. This is already designed into HoleHeader above. No blocking issue for Phase 2.

3. **Pickup scoring for running display**
   - What we know: Pickup = +2 for WHS calculation (net double bogey). But WHS uses per-hole handicap strokes which are not available during scoring (they require handicap index + SI computation).
   - What's unclear: Should the running score show pickup as +2 always, or try to compute net double bogey?
   - Recommendation: Show pickup as +2 (par + 2 offset) for the running score. The real WHS pickup value (net double bogey) is computed only in Phase 3 Round Review where handicap index is available. Mark as [ASSUMED] A2 above.
